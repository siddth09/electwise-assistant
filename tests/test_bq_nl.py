...
with patch("google.cloud.bigquery.Client"), patch(
    "google.cloud.language_v2.LanguageServiceClient"
):
    from app import (
...
def test_is_safe_input_disabled():
    app._NL_OK = False
    assert _is_safe_input("This is a test message that is long enough") is True
...
def test_is_safe_input_enabled_unsafe():
    ...
    assert (
        _is_safe_input("This is a test message that is long enough") is False
    )
